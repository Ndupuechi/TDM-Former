...
# ================================================================================================
# 🏷️ ============ 📝 Model Summary Logging (FULL) ===============================================
# ================================================================================================
def save_model_summary_full(
    baseline_model,    
    model,
    task,
    train_dataset,
    ...
    if exp_args.decimal_places == "two":
        ...
        model_summary.append(
        f"📦 Params: {model_params} M | "
        f"⚙️ MACs: {model_macs} GMACs | "
        f"🔥 FLOPs: {model_flops} GFLOPS"
        )
    # ─────────────────────────────────────────────────────────────────────────────────────────────────
    elif exp_args.decimal_places == "four":
    ...
    if exp_args.net == "TDM_Former":
        ...
        model_summary.append("------------------------------------------")
        model_summary.append("TDM Parameters")
        model_summary.append("------------------------------------------")

        tdm_log = (
            f"🔧 tdm_alpha={exp_args.tdm_alpha} | "
            f"🔧 tdm_transition_amplification={exp_args.tdm_transition_amplification} | "
            f"🔧 tdm_num_layers={exp_args.tdm_num_layers} | "
            f"🧬 tdm_transition={exp_args.tdm_transition} | "
            f"🌀 tdm_rotation={exp_args.tdm_rotation} | "
            f"⏪⏭️ tdm_insertion_type={exp_args.tdm_insertion_type} | "
            f"📐 tdm_scale_init={exp_args.tdm_scale} | "
            f"🚦 tdm_start_epoch={exp_args.tdm_start_epoch} | "
            f"🏁 tdm_full_epoch={exp_args.tdm_full_epoch} | "
            f"🚪 tdm_gate_type={exp_args.tdm_gate_type}"
        )

        model_summary.append(tdm_log)
        model_summary.append("")
    # ─────────────────────────────────────────────────────────────────────────────────────────────────
