...
# ────────────────────────────────────────────────────────────────────────────────────────────────
# 1️⃣✅ Import parser from parser_IWSLT14De_En.py
...
########################################################################################################################
########################################################################################################################
####-------| NOTE 3.1. PROCESSING DATASET 1 | XXX ------------| GERMAN→ENGLISH |--------------------####################
########################################################################################################################

"""
Stage 1:
Train SentencePiece tokenizer, encode the raw German→English corpus,
...
if exp_args.run_dataset_tokenization:
    ...
    # ─────────────────────────────────────────────────────────────────────────────────────────────────
    # ✅ Path for DE→EN dataset
    DATA_PATH = r"C:\Users\emeka\Research\ModelCUDA\Transformers\Transformer_Baselines_DE_EN\Dataset\en-de\de-en"
    os.makedirs(DATA_PATH, exist_ok=True)
    # ─────────────────────────────────────────────────────────────────────────────────────────────────

    # ─────────────────────────────────────────────────────────────────────────────────────────────────
    #  1️⃣ ======== Official IWSLT14 training files ==================================================
    # ─────────────────────────────────────────────────────────────────────────────────────────────────

    SRC_DE = os.path.join(DATA_PATH, "train.tags.de-en.de")
    TGT_EN = os.path.join(DATA_PATH, "train.tags.de-en.en")

    assert os.path.exists(SRC_DE), f"Missing: {SRC_DE}"
    assert os.path.exists(TGT_EN), f"Missing: {TGT_EN}"

    print("✅ Using official IWSLT14 training files")
    print(f"🇩🇪 German : {SRC_DE}")
    print(f"🇬🇧 English: {TGT_EN}")
    # ─────────────────────────────────────────────────────────────────────────────────────────────────

    # ─────────────────────────────────────────────────────────────────────────────────────────────────
    #  2️⃣ ======== Extract official validation/test XML files ========================================
    # ─────────────────────────────────────────────────────────────────────────────────────────────────
    def extract_xml_sentences(xml_file, out_file):

        assert os.path.exists(xml_file), f"Missing: {xml_file}"

        with open(xml_file, "r", encoding="utf-8") as fin:
            text = fin.read()
        ...
        with open(out_file, "w", encoding="utf-8") as fout:
            for seg in segs:
                seg = re.sub(r"\s+", " ", seg)
                seg = html.unescape(seg.strip())
                fout.write(seg + "\n")

        print(f"✅ Extracted {len(segs)} lines: {out_file}")

    VALID_EN = os.path.join(DATA_PATH, "valid.en")
    VALID_DE = os.path.join(DATA_PATH, "valid.de")
    TEST_EN  = os.path.join(DATA_PATH, "test.en")
    TEST_DE  = os.path.join(DATA_PATH, "test.de")

    extract_xml_sentences(
        os.path.join(DATA_PATH, "IWSLT14.TED.dev2010.de-en.en.xml"),
        VALID_EN
    )

    extract_xml_sentences(
        os.path.join(DATA_PATH, "IWSLT14.TED.dev2010.de-en.de.xml"),
        VALID_DE
    )

    extract_xml_sentences(
        os.path.join(DATA_PATH, "IWSLT14.TED.tst2010.de-en.en.xml"),
        TEST_EN
    )

    extract_xml_sentences(
        os.path.join(DATA_PATH, "IWSLT14.TED.tst2010.de-en.de.xml"),
        TEST_DE
    )
    # ─────────────────────────────────────────────────────────────────────────────────────────────────

    # ─────────────────────────────────────────────────────────────────────────────────────────────────
    #  3️⃣ ======== Train SentencePiece ===============================================================
    # ─────────────────────────────────────────────────────────────────────────────────────────────────
    print("🧠 Training SentencePiece model for DE→EN...")

    spm_model_prefix = os.path.join(DATA_PATH, "spm_ende")

    spm.SentencePieceTrainer.Train(
        input=f"{SRC_DE},{TGT_EN}",
    ...
    # ─────────────────────────────────────────────────────────────────────────────────────────────────
    #  4️⃣ ======== Encode train/valid/test files =====================================================
    # ─────────────────────────────────────────────────────────────────────────────────────────────────
    ...
    encode_file(
        SRC_DE,
        os.path.join(DATA_PATH, "train.spm.de")
    )

    encode_file(
        TGT_EN,
        os.path.join(DATA_PATH, "train.spm.en")
    )

    encode_file(
        VALID_EN,
        os.path.join(DATA_PATH, "valid.spm.en")
    )

    encode_file(
        VALID_DE,
        os.path.join(DATA_PATH, "valid.spm.de")
    )

    encode_file(
        TEST_EN,
        os.path.join(DATA_PATH, "test.spm.en")
    )

    encode_file(
        TEST_DE,
        os.path.join(DATA_PATH, "test.spm.de")
    )

    print("✅ SentencePiece encoding complete.")
...
########################################################################################################################
########################################################################################################################
####-------| NOTE 3.2. PROCESSING DATASET 2 | XXX ------------| GERMAN→ENGLISH |--------------------####################
########################################################################################################################

"""
Stage 2:
Preprocess the tokenized German→English dataset using fairseq-preprocess
to build vocabularies and generate Fairseq binary training files.
"""
# ⏭️🔖for reverse translation DE→EN, you need to re-run Fairseq preprocessing, because the binary data must be built accordingly
...
if exp_args.run_fairseq_preprocessing:
    ...
    DATA_DIR = r"C:\Users\emeka\Research\ModelCUDA\Transformers\Transformer_Baselines_DE_EN\Dataset\en-de\de-en"

    required_files = [
        os.path.join(DATA_DIR, "train.spm.en"),
        os.path.join(DATA_DIR, "train.spm.de"),
        os.path.join(DATA_DIR, "valid.spm.en"),
        os.path.join(DATA_DIR, "valid.spm.de"),
        os.path.join(DATA_DIR, "test.spm.en"),
        os.path.join(DATA_DIR, "test.spm.de"),
    ]
    # ─────────────────────────────────────────────────────────────────────────────────────────────────
    ...
    print("🚀 Running fairseq-preprocess for DE→EN...")
    ...
    print("✅ Preprocessing complete! Binarized DE→EN data saved in data-bin/iwslt14.de-en")
    # ─────────────────────────────────────────────────────────────────────────────────────────────────
    # ─────────────────────────────────────────────────────────────────────────────────────────────────
...
# ✅✅ Print translation direction (DE→EN confirmation)
...
# ────────────────────────────────────────────────────────────────────────────────────────────────
if not os.path.exists(f'./Results_Architecture_Summary/{exp_args.dataset_name}/{exp_args.net}'):
    os.makedirs(f'./Results_Architecture_Summary/{exp_args.dataset_name}/{exp_args.net}')
# ────────────────────────────────────────────────────────────────────────────────────────────────

# ────────────────────────────────────────────────────────────────────────────────────────────────
# 🟡 === Efficiency Test  directory ===
# ────────────────────────────────────────────────────────────────────────────────────────────────
if not os.path.exists(f'./Results_efficiency_test/{exp_args.dataset_name}/{exp_args.net}'):
    os.makedirs(f'./Results_efficiency_test/{exp_args.dataset_name}/{exp_args.net}')
# ────────────────────────────────────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────────────────────────────────────
# 2️⃣.1️⃣🔖 ==== Checkpoint Directory Selection | Normal Run vs Crash Run vs Efficiency Checkpoint Test | save every epoch ===== 
# ────────────────────────────────────────────────────────────────────────────────────────────────

# 🅰️💾 Normal run: save checkpoints in standard folder
if not exp_args.crash_run and not exp_args.run_efficiency_test:
...
elif exp_args.crash_run and not exp_args.run_efficiency_test:
    checkpoint_dir = f'./checkpoints_crash_run/{exp_args.dataset_name}/{exp_args.net}/'

    # 📁 Create checkpoint folder if it does not exist    
    os.makedirs(checkpoint_dir, exist_ok=True)

    print(f"🅱️♻️♻️ Crash Run Enabled | Saving recovery checkpoints to: {checkpoint_dir}")

#---------------------------------------------------------------------------------------------------
# 🅲🔼 Efficiency checkpoint test: load checkpoints from efficiency-test folder
elif exp_args.run_efficiency_test and not exp_args.crash_run:
    checkpoint_dir = f'./checkpoints_efficiency_test_run/{exp_args.dataset_name}/{exp_args.net}/{exp_args.mode_name}/'

    # 📁  Create checkpoint folder if it does not exist 
    os.makedirs(checkpoint_dir, exist_ok=True)

    print(f"🅲⚖️⚖️ Efficiency Checkpoint Test Enabled | Loading checkpoints from: {checkpoint_dir}")

#---------------------------------------------------------------------------------------------------
# 🚫 Invalid mode
else:
    raise ValueError(
        "🚫🚫 Incorrect checkpoint mode selected: crash_run=True and run_efficiency_test=True cannot both be enabled."
    )
...
model_summary_path = f'./Results_Architecture_Summary/{exp_args.dataset_name}/{exp_args.net}/{exp_args.net}_{exp_args.dataset_name}_{exp_args.optimizer1}_{exp_args.mode_name}_model_summary.txt'
model_summary_full_path = f'./Results_Architecture_Summary/{exp_args.dataset_name}/{exp_args.net}/{exp_args.net}_{exp_args.dataset_name}_{exp_args.optimizer1}_{exp_args.mode_name}_model_summary_full.txt'
# ────────────────────────────────────────────────────────────────────────────────────────────────
# 2️⃣.5️⃣🔖 ===  Efficiency Test path === 
efficiency_results_path = f'./Results_efficiency_test/{exp_args.dataset_name}/{exp_args.net}/{exp_args.net}_{exp_args.dataset_name}_{exp_args.optimizer1}_{exp_args.mode_name}_efficiency_test.txt'
...
def test_checkpoint_epochs(task, model, criterion, checkpoint_dir, test_checkpoint_results_path, efficiency_results_path):
    """
...
# ────────────────────────────────────────────────────────────────────────────────────────────────
    ...
    # ================================================================================================
    # 🅰️🔼 MODE: EFFICIENCY TEST RUN 🔼🅰️       📦 Reset output file + write model status   
    # ────────────────────────────────────────────────────────────────────────────────────────────────
    # ────────────────────────────────────────────────────────────────────────────────────────────────
    # 🅰️🔼 MODE: EFFICIENCY TEST RUN 🔼🅰️       📦 Reset output file + write model status
    # ================================================================================================
    if exp_args.run_efficiency_test:

        os.makedirs(os.path.dirname(efficiency_results_path), exist_ok=True)

        if not exp_args.crash_run:

            with open(efficiency_results_path, "w", encoding="utf-8") as f:
                f.write("")

        else:

            with open(efficiency_results_path, "a", encoding="utf-8") as f:
                f.write("\n♻️ Resuming efficiency testing after crash\n\n")
    # ================================================================================================


    # ================================================================================================
    #  🅱️🔼 MODE: NORMAL RUN 🔼🅱️            📦 Reset output file + write model status  
    # ────────────────────────────────────────────────────────────────────────────────────────────────
    # ────────────────────────────────────────────────────────────────────────────────────────────────
    #  🅱️🔼 MODE: NORMAL RUN 🔼🅱️            📦 Reset output file + write model status 
    # ================================================================================================
        ...
        with open(efficiency_results_path, "a", encoding="utf-8") as f:
            ...
            if exp_args.net == "TDM_Former":
                ...
                f.write(
                    ...
                    f"(L{6-exp_args.tdm_num_layers+1}-L6) | "
                    f"insertion={exp_args.tdm_insertion_type}\n"
                    f"Efficiency Test: True\n"
                    f"Metrics: BLEU | Inference Time (s) | Throughput (words/s) | Memory (GB) | Beam\n\n"
                )
    ...
    for ckpt in ckpts:
        ...
        with torch.no_grad():
            with tqdm(enumerate(test_iter), total=len(test_iter), desc=f"Testing {os.path.basename(ckpt)}") as progress:
                for i, sample in progress:   # 👈 keep i!
                    ...
                    # ================================================================================================
                    # 🅰️🔼 MODE: EFFICIENCY TEST RUN 🔼🅰️     📦♻️ Decoding output block
                    # ────────────────────────────────────────────────────────────────────────────────────────────────
                    # ────────────────────────────────────────────────────────────────────────────────────────────────
                    # 🅰️🔼 MODE: EFFICIENCY TEST RUN 🔼🅰️     📦♻️ Decoding output block
                    # ================================================================================================
                    if exp_args.run_efficiency_test:
                        ...
                        for j, hypos_j in enumerate(hypos):
                            ...
                            sys_str = tgt_dict.string(hypo_tokens, bpe_symbol="@@")
                            sys_str = detokenize_for_bleu(sys_str)

                            sys_outputs.append(sys_str)
                            total_words += len(sys_str.split())

                        # ✅ Update progress bar postfix with generated word count
                        progress.set_postfix(words=total_words)
                    # ================================================================================================


                    # ================================================================================================
                    # 🅰️🔼 MODE: NORMAL RUN  🔼🅰️          📦♻️ Decoding output block
                    # ────────────────────────────────────────────────────────────────────────────────────────────────
                    # ────────────────────────────────────────────────────────────────────────────────────────────────
                    # 🅰️🔼 MODE: NORMAL RUN  🔼🅰️          📦♻️ Decoding output block
                    # ================================================================================================
       ...
       # ────────────────────────────────────────────────────────────────────────────────────────────────
        ...
        # ================================================================================================
        # 🅰️🔼 MODE: EFFICIENCY TEST RUN 🔼🅰️     📦🧩 Metric calculation : BLEU, THROUGHPUT, MEMORY 
        # ────────────────────────────────────────────────────────────────────────────────────────────────
        # ────────────────────────────────────────────────────────────────────────────────────────────────
        # 🅰️🔼 MODE: EFFICIENCY TEST RUN 🔼🅰️     📦🧩 Metric calculation : BLEU, THROUGHPUT, MEMORY 
        # ================================================================================================
        if exp_args.run_efficiency_test:
            ...
            # 🚫🔒 LOSS not computed in MODE: EFFICIENCY TEST RUN
            avg_loss = 0.0

            # ===================================================
            # 🕒🕒 ==== INFERENCE TIME (s) =====================
            # ===================================================
            # 🏷️ total decoding/evaluation loop time for this checkpoint/epoch checkpoint, before BLEU calculation
            # 🏷️ duration = current time after full test decoding - start time before decoding loop
            duration = time.time() - start_time
            total_test_duration += duration
            ...
            # 🔍🔍 ==== THROUGHPUT (words/s) ===================
            # ===================================================
            # 🏷️generated output words per second
            # 🏷️throughput_words_per_sec = total_words / duration ➡️ total generated words ÷ total seconds used
            ...
            # 📦📦 ==== GPU MEMORY (GB) ========================
            # ===================================================
            # 🏷️Peak allocated GPU memory during decoding
            # 🏷️peak PyTorch-allocated CUDA memory during this checkpoint decoding/test run
            # 🏷️memory_gb = maximum GPU memory allocated by PyTorch during this checkpoint test run
            memory_gb = (
        ...
        # ================================================================================================
        # 🅰️🔼 MODE: NORMAL RUN  🔼🅰️          📦🧩 Metric calculation : LOSS, BLEU
        # ────────────────────────────────────────────────────────────────────────────────────────────────
        # ────────────────────────────────────────────────────────────────────────────────────────────────
        # 🅰️🔼 MODE: NORMAL RUN  🔼🅰️          📦🧩 Metric calculation : LOSS, BLEU
        # ================================================================================================
        ...
        # ================================================================================================
        # 🅰️🔼 MODE: EFFICIENCY TEST RUN 🔼🅰️     📦✍️ Checkpoint result writing
        # ────────────────────────────────────────────────────────────────────────────────────────────────
        # ────────────────────────────────────────────────────────────────────────────────────────────────
        # 🅰️🔼 MODE: EFFICIENCY TEST RUN 🔼🅰️     📦✍️ Checkpoint result writing
        # ================================================================================================
        if exp_args.run_efficiency_test:
            ...
            epoch_to_write = state.get("epoch", extract_epoch_num(os.path.basename(ckpt)))

            with open(efficiency_results_path, "a", encoding="utf-8") as f:
                f.write(
                    f"Epoch: {epoch_to_write} "
                    f"| Checkpoint {os.path.basename(ckpt)} "
                    f"| BLEU: {bleu.score:.2f} "
                    f"| Inference Time (s): {duration:.2f} "
                    f"| Throughput (words/s): {throughput_words_per_sec:.2f} "
                    f"| Memory (GB): {memory_gb:.2f} "
                    f"| Beam={exp_args.beam}\n"
                )

            print(
                f"✅✅ Epoch {epoch_to_write} "
                f"| ✔️✔️ {os.path.basename(ckpt)} "
                f"| BLEU {bleu.score:.2f} "
                f"| Time {duration:.2f}s "
                f"| Throughput {throughput_words_per_sec:.2f} words/s "
                f"| Mem {memory_gb:.2f} GB "
                f"| Beam {exp_args.beam}"
            )

            # 🧹 add visual spacing between efficiency-test runs
            tqdm.write("")
        # ================================================================================================


        # ================================================================================================
        # 🅰️🔼 MODE: NORMAL RUN  🔼🅰️          📦✍️ Checkpoint result writing
        # ────────────────────────────────────────────────────────────────────────────────────────────────
        # ────────────────────────────────────────────────────────────────────────────────────────────────
        # 🅰️🔼 MODE: NORMAL RUN  🔼🅰️          📦✍️ Checkpoint result writing
        # ================================================================================================
    ...
    # ================================================================================================
    # 🅰️🔼 MODE: EFFICIENCY TEST RUN 🔼🅰️     📝✍️ Final summary write
    # ────────────────────────────────────────────────────────────────────────────────────────────────
    # ────────────────────────────────────────────────────────────────────────────────────────────────
    # 🅰️🔼 MODE: EFFICIENCY TEST RUN 🔼🅰️     📝✍️ Final summary write
    # ================================================================================================
    if exp_args.run_efficiency_test:
        ...
        with open(efficiency_results_path, "a", encoding="utf-8") as f:
            ...
            total_mins, total_secs = divmod(total_test_duration, 60)
            f.write(f"\n🕒 Total Efficiency Test Time over all checkpoints: {int(total_mins)} min {total_secs:.2f} sec\n")

        print(f"🏆 Best BLEU over efficiency-test checkpoints: {best_bleu:.2f}")
        print(f"🕒 Total Efficiency Test Time over all checkpoints: {int(total_mins)} min {total_secs:.2f} sec")
        print(f"📜 Efficiency test logs saved to {efficiency_results_path}")

        # 🧹 add visual spacing after final efficiency summary
        tqdm.write("")
    # ================================================================================================


    # ================================================================================================
    # 🅰️🔼 MODE: NORMAL RUN  🔼🅰️          📝✍️ Final summary write
    # ────────────────────────────────────────────────────────────────────────────────────────────────
    # ────────────────────────────────────────────────────────────────────────────────────────────────
    # 🅰️🔼 MODE: NORMAL RUN  🔼🅰️          📝✍️ Final summary write
    # ================================================================================================
...
test_checkpoint_epochs(task, model, criterion, checkpoint_dir, test_checkpoint_results_path, efficiency_results_path)
# ────────────────────────────────────────────────────────────────────────────────────────────────
# ================================================================================================
