...
def save_model_summary_full(
    ...
    if exp_args.net == "RTM_Former":
        ...
        rtm_log = (
            f"🔧 rtm_alpha={exp_args.rtm_alpha} | "
            f"🔧 rtm_transition_amplification={exp_args.rtm_transition_amplification} | "
            f"🔧 rtm_num_layers={exp_args.rtm_num_layers} | "
            f"⏪⏭️ rtm_insertion_type={exp_args.rtm_insertion_type} | "
            f"📐 rtm_scale_init={exp_args.rtm_scale} | "
            f"🚦 rtm_start_epoch={exp_args.rtm_start_epoch} | "
            f"🏁 rtm_full_epoch={exp_args.rtm_full_epoch}"
        )

        if exp_args.rtm_insertion_type == "ffn_depth_weighted":

            rtm_log += (
                f" | 📊 rtm_layer_scale_min={exp_args.rtm_layer_scale_min}"
                f" | 🔁 rtm_layer_scale_direction={exp_args.rtm_layer_scale_direction}"
                f" | 🚪 rtm_gate_type={exp_args.rtm_gate_type}"
            )

            if exp_args.rtm_layer_scale_direction == "reverse_final_fixed":
                rtm_log += (
                    f" | 🔒 rtm_final_layer_scale={exp_args.rtm_final_layer_scale}"
                )

        model_summary.append(rtm_log)
        model_summary.append("")
    # ─────────────────────────────────────────────────────────────────────────────────────────────────
