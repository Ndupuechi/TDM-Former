...
Project_PATH = r"C:\Users\emeka\Research\ModelCUDA\Transformers\Transformer_Baselines_RO_EN"
...
# ────────────────────────────────────────────────────────────────────────────────────────────────
# 1️⃣✅ Import parser from parser_IWSLT17Ro_En.py
from parser_IWSLT17Ro_En import get_parser
...
from fairseq_config_IWSLT17Ro_En import get_fairseq_parser
...
########################################################################################################################
########################################################################################################################
####-------| NOTE 3.1. PROCESSING DATASET 1 | XXX ------------| ROMANIAN→ENGLISH |------------------####################
########################################################################################################################

"""
Stage 1:
Train SentencePiece tokenizer, encode the raw English→Romanian corpus,
...
if exp_args.run_dataset_tokenization:
    ...
    # ===============================================================================================
    DATA_PATH = r"C:\Users\emeka\Research\ModelCUDA\Transformers\Transformer_Baselines_RO_EN\Dataset\en-ro\en-ro"
    os.makedirs(DATA_PATH, exist_ok=True)

    print("📥 Downloading/loading IWSLT17 EN↔RO dataset...")
    ...
    def write_split(split_name, out_prefix):
        ...
        ro_path = os.path.join(DATA_PATH, f"{out_prefix}.ro")

        with open(en_path, "w", encoding="utf-8") as f_en, \
             open(ro_path, "w", encoding="utf-8") as f_ro:

            for sample in dataset[split_name]:
                f_en.write(sample["translation"]["en"].strip() + "\n")
                f_ro.write(sample["translation"]["ro"].strip() + "\n")
    ...
    # ===============================================================================================
    SRC_RO = os.path.join(DATA_PATH, "train.ro")
    TGT_EN = os.path.join(DATA_PATH, "train.en")

    VALID_RO = os.path.join(DATA_PATH, "valid.ro")
    VALID_EN = os.path.join(DATA_PATH, "valid.en")

    TEST_RO = os.path.join(DATA_PATH, "test.ro")
    TEST_EN = os.path.join(DATA_PATH, "test.en")

    assert os.path.exists(SRC_RO), f"Missing: {SRC_RO}"
    assert os.path.exists(TGT_EN), f"Missing: {TGT_EN}"

    assert os.path.exists(VALID_RO), f"Missing: {VALID_RO}"
    assert os.path.exists(VALID_EN), f"Missing: {VALID_EN}"

    assert os.path.exists(TEST_RO), f"Missing: {TEST_RO}"
    assert os.path.exists(TEST_EN), f"Missing: {TEST_EN}"

    # ===============================================================================================
    # 4️⃣ Train SentencePiece
    # ===============================================================================================
    print("🧠 Training SentencePiece model for RO→EN...")

    spm_model_prefix = os.path.join(DATA_PATH, "spm_roen")

    spm.SentencePieceTrainer.Train(
        input=f"{SRC_RO},{TGT_EN}",
    ...
    encode_file(SRC_RO, os.path.join(DATA_PATH, "train.spm.ro"))
    encode_file(TGT_EN, os.path.join(DATA_PATH, "train.spm.en"))

    encode_file(VALID_RO, os.path.join(DATA_PATH, "valid.spm.ro"))
    encode_file(VALID_EN, os.path.join(DATA_PATH, "valid.spm.en"))

    encode_file(TEST_RO, os.path.join(DATA_PATH, "test.spm.ro"))
    encode_file(TEST_EN, os.path.join(DATA_PATH, "test.spm.en"))

    print("✅ RO→EN SentencePiece encoding complete")
...
########################################################################################################################
########################################################################################################################
####-------| NOTE 3.2. PROCESSING DATASET 2 | XXX ------------| ROMANIAN→ENGLISH |------------------####################
########################################################################################################################

"""
Stage 2:
Preprocess the tokenized Romanian→English dataset using fairseq-preprocess
...
if exp_args.run_fairseq_preprocessing:
    ...
    DATA_DIR = r"C:\Users\emeka\Research\ModelCUDA\Transformers\Transformer_Baselines_RO_EN\Dataset\en-ro\en-ro"


    print("🚀 Running fairseq-preprocess IWSLT17 RO→EN")
    ...
    cmd = [
        ...
        "--destdir", "data-bin/iwslt17.ro-en",
    ...
    print("✅ data-bin/iwslt17.ro-en created")
    # ─────────────────────────────────────────────────────────────────────────────────────────────────
    # ─────────────────────────────────────────────────────────────────────────────────────────────────
...
# ✅✅ Print translation direction (RO→EN confirmation)
...
# ================================================================================================
    ...
    # ───────────────────────────────────────────────────────────────────────────────────────────────
    # 2️⃣ 🧩🧠 === Cross-Attention Insertion ♻️♻️ ================================================= 
    # ───────────────────────────────────────────────────────────────────────────────────────────────
    # ───────────────────────────────────────────────────────────────────────────────────────────────
    # 2️⃣ 🧩🧠 === Cross-Attention Insertion ♻️♻️ ================================================= 
    # ───────────────────────────────────────────────────────────────────────────────────────────────
    ...
    # ───────────────────────────────────────────────────────────────────────────────────────────────
    # 3️⃣ 🧩🧠 === FFN Depth-Weighted Insertion === ♻️♻️ =========================================== 
    # ───────────────────────────────────────────────────────────────────────────────────────────────
    # ───────────────────────────────────────────────────────────────────────────────────────────────
    # 3️⃣ 🧩🧠 === FFN Depth-Weighted Insertion === ♻️♻️ =========================================== 
    # ───────────────────────────────────────────────────────────────────────────────────────────────
        ...
        with open(train_results_path, "w", encoding="utf-8") as f:
            ...
            if exp_args.net == "RTM_Former":
                ...
                f.write(
                    f"Model: RTM_Former\n"
                    f"Status: "
                    f"rotation={exp_args.rtm_rotation} | "
                    f"depth_weight={exp_args.rtm_depth_weight} | "
                    f"gate={exp_args.rtm_gate_type} | "
                    f"insertion={exp_args.rtm_insertion_type}\n\n"
                )
...
# ─────────────────────────────────────────────────────────────────────────────────────────────────
#  1️⃣ ========  9.1  BLEU Detokenization  (🔖 EN/FR/DE/IT/RO) ====================================
# ───────────────────────────────────────────────────────────────────────────────────────────────── 
# ────────────────────────────────────────────────────────────────────────────────────────────────
...
def test_one_epoch(epoch, task, model, criterion, beam_size=None):
    ...
    bleu = sacrebleu.corpus_bleu(sys_outputs, [true_references])      #🔖 EN/FR/DE/IT/RO
...
if __name__ == "__main__":
    ...
    # ────────────────────────────────────────────────────────────────────────────────────────────────
        ...
        for epoch in range(exp_args.start_epoch, exp_args.epochs):
            ...
            if epoch < RTM_START_EPOCH:
                RTM_SCALE = exp_args.rtm_scale
            else:
                progress = min(
                    1.0,
                    (epoch - RTM_START_EPOCH) / (RTM_FULL_EPOCH - RTM_START_EPOCH)
                )

                RTM_SCALE = 0.5 * (1.0 - math.cos(math.pi * progress))

            print(f"📐 RTM_SCALE for epoch {epoch}: {RTM_SCALE:.3f}")
            #------------------------------------------------------------------------------------------
            #------------------------------------------------------------------------------------------
            if exp_args.rtm_insertion_type == "ffn_depth_weighted":
                ...
                if exp_args.rtm_layer_scale_direction == "forward":
                    depth_scales = [
                        exp_args.rtm_layer_scale_min + (1.0 - exp_args.rtm_layer_scale_min) * (j / max(1, exp_args.rtm_num_layers - 1))
                ...
                # ===================================================== 
                    ...
                    depth_scales = [
                        1.0 - (1.0 - exp_args.rtm_layer_scale_min) * (j / max(1, exp_args.rtm_num_layers - 1))
                        for j in range(exp_args.rtm_num_layers)
                    ]
                ...
                # print(f"📐 Final RTM layer scales ({exp_args.rtm_layer_scale_direction}): {[round(s,3) for s in final_scales]}")
            ...
            #------------------------------------------------------------------------------------------
            # ───────────────────────────────────────────────────────────────────────────────
            # 🤖🧠 RTM_Former Training | Rotational Transition Learning 🌀📈
            # ───────────────────────────────────────────────────────────────────────────────            
            train_loss = train_one_epoch(epoch, task, model, optimizer, criterion, lr_scheduler)
            tqdm.write("")
            # ─────────────────────────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────────────────────────
#  2️⃣ ========  BLEU Detokenization (🔖 EN/FR/DE/IT/RO) ==========================================
# ───────────────────────────────────────────────────────────────────────────────────────────────── 
# ────────────────────────────────────────────────────────────────────────────────────────────────
